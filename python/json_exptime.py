#!/usr/bin/python
"""
Exposure time / Signal to Noise calculator for the red and
blue channel spectrographs on the MMT.
This is Version 0.0 of the code - expect substantial structural c
hanges to integrate with the PHP for use online.
...
def extrap1d(interpolator):
    ...
    def pointwise(x):
        if x < xs[0]:
            yval = ys[0] + (x - xs[0]) * (ys[1] - ys[0]) / (xs[1] - xs[0])
        elif x > xs[-1]:
            yval = ys[-1] + (x - xs[-1]) * (ys[-1] - ys[-2]) / \
                   (xs[-1] - xs[-2])
        else:
            return interpolator(x)

        # Don't allow negative extrapolations
        if yval > 0:
            return yval
        else:
            return 1e-10
    ...
    def ufunclike(xs):
        return np.array([float(x) for x in map(pointwise, np.array(xs))])
...
def getFilterCorrection(opt, lam):
    ...
    if filterName == "Clear":
        filtThrough = []
        # we don't have a file for clear, so just construct it
        filtLam = np.arange(3000, 1e4)
    ...
    filtInterpol = cleanThroughputCorrections(
        lam, filtInterpol, filtLam, filtThrough)

    return filtInterpol / 100.


def getBlazeCorrection(opt, lam):
    """Return blaze correction term.

    This function will look at the grating requested and make a
    correction between that and the nominal grating
    that the throughput was defined for and calculate the correction term
    ...
    blazeFilename = 'data/spectrodata/grating/' + instr + '_' + \
                    grating + '.dat'
    refFilename = 'data/spectrodata/grating/' + \
        instr + '_' + referenceGrating + '.dat'

    blazeLam, blazeThrough = readSpectroDataFile(blazeFilename)
    refLam, refThrough = readSpectroDataFile(refFilename)

    # Need to do the interpolation and extrapolation before the ratio to
    # make sure we're on the same pixel coordinates
    blazeInterpol = interpol(lam, blazeLam, blazeThrough)
    blazeInterpol = cleanThroughputCorrections(
        lam, blazeInterpol, blazeLam, blazeThrough)

    refInterpol = interpol(lam, refLam, refThrough)
    refInterpol = cleanThroughputCorrections(
        lam, refInterpol, refLam, refThrough)

    # Now, we need to multiply by the blazeInterpol, but divide by the
    # reference
...
def cleanThroughputCorrections(lam, fullInterpol, fullLam, fullThrough):
    # This is just a function to do a repeated task of extrapolating
    # throughput currves
    ...
    for ii in range(0, len(lam)):
        ...
        if (fullInterpol[ii] <= 0):
            # I do this to remove divide by zero errors
            fullInterpol[ii] = 1e-9
    return fullInterpol


def readSpectroDataFile(filename):
    # Quick wrapper to read the points for the spectrograph data (x,y)
...
def read_options(filename):
    # Read the filename into a dictionary.
    # The assumed format is white space separation
...
def read_skymodels(opt):
    """Read in the archived sky models.

    Read the ascii files with each of the sky models
    (for different lunar phases) in them
    Based on the set options, interpolate the models to the
    correct lunar phase and return
    """
    ...
    if below == junk:
        # Since the index was exactly the same as
        # one of the input indexes, then
        ...
        for ii in range(len(flux1)):
            flux[ii] = (flux1[ii] - flux1[ii]) / \
                        (lunarphase[top] - lunarphase[below]) *\
                        (float(opt['lunar_phase']) - lunarphase[below]) + \
                        flux1[ii]
...
def get_lam(opt):
    ...
    wave = (np.arange(0, npix) - (npix / 2.)) * \
        disperse + float(opt['cenwave'])
...
def get_resolution(opt):
    ...
    grating_param = grating_params(opt)
    resolution = float(opt['cenwave']) / \
        grating_param[0] * float(opt['slit_width'])
    return resolution
...
def calculate_sky(opt):
    ...
    # Here, we estimate the efficicy based on the count
    # rate (since the count rate is of a known source
    ...
    e_lam = input[0]
    # This second term is the expected count rate for a 1uJY source
    e_frac1 = input[1] * (0.002658 * e_lam)
    ...
    lam_out = get_lam(opt)
    skyspec_out = interpol(lam_out, lam, skyspec) * grating_param[1] * float(
        opt['binning_spectral'])  # Photons/s/pixel (instead of Angstrom)
...
def slit_transmission(opt):
    ...
    trans = scipy.special.erf(
        float(opt['slit_width']) / (2 * np.sqrt(2.) *
                                    float(opt['seeing']) / 2.35482))
    return trans
...
def source_transmission(opt):
    ...
    input = np.genfromtxt('data/throughput/kpnoextinct.dat',
    ...
    # photons (because of gain correction) / s / Angstrom
    flux = flux * eff_corr * get_gain(opt)
    ...
    flux = gauss_convol(lam, flux, resolution) * grating_param[1] * float(
        opt['binning_spectral'])  # Convolved spectrum in photons / s / pixel
    return [lam, flux]
...
def main(argv):
    ...
    npix = math.ceil(
        2 * float(opt['seeing']) / (float(opt['binning_spatial']) *
                                    get_pixel_scale()))
    area_sky = npix * float(opt['slit_width']) * \
        get_pixel_scale() * float(opt['binning_spectral'])
    ...
    # Now, Check to see if we were asked to calculate the
    # signal to noise in a given time or the
    ...
    if opt['calc_snr'] == '1':
        ...
        snr = objflux * exptime / \
            (objflux * exptime + skyflux * npix *
             exptime + readnoise**2 * npix)**0.5
    ...
    if opt['calc_time'] == '1':
        ...
        exptime = (snr_goal**2 * (objflux + npix * skyflux) +
                   (snr_goal**4 * (objflux + npix * skyflux)**2 +
                    4.0 * objflux**2 * snr_goal**2 *
                    npix * readnoise**2)**0.5) / (2 * objflux**2)
        ...
        for ii in range(0, len(lam)):
            if (lam[ii] > minwave) and (lam[ii] < maxwave) and \
               (exptime[ii] > 0):
